...
export default function YourRidesClient({ initialRides }: { initialRides: Ride[] }) {
  ...
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
      {rides.map((ride) => (
        <RideCard key={ride.id} ride={ride} onUpdate={(updated) => {
          setRides((prev) =>
...
function ManageRequests({
  ...
  function handleAcceptRequest(name: string) {
    ...
    onUpdate({ ...ride, occupantNames: updatedOccupants, requests: updatedRequests })
    toast({ title: "Request accepted", description: `Added ${name} to your ride.` })
  }

  function handleRejectRequest(name: string) {
    const updatedRequests = ride.requests.filter((r) => r.name !== name)
    onUpdate({ ...ride, requests: updatedRequests })
    toast({ title: "Request rejected", description: `Rejected ${name}'s request.` })
  }
    ...
    <div>
      ...
      <ul className="space-y-2">
        {ride.requests.map((req) => (
          <li key={req.name} className="flex items-center justify-between bg-muted/30 p-2 rounded">
            <div>
            ...
            <div className="flex items-center gap-2">
              <Button variant="outline" size="icon" onClick={() => handleRejectRequest(req.name)}>
                <X className="h-4 w-4" />
              </Button>
              <Button variant="outline" size="icon" onClick={() => handleAcceptRequest(req.name)}>
...
function ManageRiders({
  ...
  function handleMessageRider(name: string) {
    toast({ title: `Message sent to ${name}`, description: "Hello from the driver!" })
  }

  return (
    <div>
      <h3 className="text-sm font-medium mb-2">Current Riders</h3>
      {ride.occupantNames.length <= 1 && ride.occupantNames[0].startsWith("You") ? (
        <p className="text-sm text-muted-foreground">No other riders yet.</p>
      ) : (
        <ul className="space-y-2">
          {ride.occupantNames.map((rider) => (
            <li key={rider} className="flex items-center justify-between bg-muted/10 p-2 rounded">
              <p className="text-sm">{rider}</p>
              {/* Right side: remove or message */}
              <div className="flex items-center gap-2">
                <Button size="icon" variant="outline" onClick={() => handleMessageRider(rider)}>
                  💬
                </Button>
                <Button
                  size="icon"
